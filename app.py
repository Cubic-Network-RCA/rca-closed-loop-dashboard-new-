...
def safe_table(rows, use_container_width=True):
    """Render a list[dict] without pandas. Falls back gracefully."""
    if not rows:
        st.info("No data.")
        return
    try:
        ...
        # fallback: render as simple text blocks
        for r in rows[:200]:
            st.write(r)
...
# ---------------------- AI similarity (open-source, lightweight) ----------------------
...
def audit_rows(rcas):
    """Return rows with action counts + missing evidence counts."""
    if not rcas:
        return []
    ...
    ev = query_rows(
        "SELECT DISTINCT action_id FROM evidence"
    )
...
st.caption(
    "Upload RCAs, auto-create remedial actions, track evidence + verification, and detect recurrence using lightweight similarity (open-source)."
)
    ...
    if up is not None:
        ...
        if st.button("Save RCA to DB"):
            if not oem.strip():
                ...
                # Create actions with default due date
                rows = []
...
with tab_audit:
    ...
    st.write("Use sidebar filter **Pre-Live last 6 months (audit)** to run your pre-Live assurance review.")

    if not rcas:
        st.info("No RCAs match your filters.")
    else:
        view = audit_rows(rcas)
        safe_table(view)
...
with tab_detail:
    ...
    if not rcas:
        ...
        # Build selection label map
        id_to_title = {r["rca_id"]: r.get("title", "") for r in rcas}
        pick = st.selectbox("Select RCA", [r["rca_id"] for r in rcas], format_func=lambda rid: f"{rid} — {id_to_title.get(rid,'')}")
    ...
    with colA:
        if st.button("Find similar RCAs"):
            if not inc_summary.strip():
                ...
                sims = top_similar_rcas(f"{inc_oem} {inc_env} {inc_sev} {inc_summary}", topk=8, oem_filter=inc_oem.strip() or None)
                if not sims:
        ...
        if st.button("Log incident"):
            if not inc_summary.strip():
                ...
                exec_sql(
                    ...
                    VALUES (?,?,?,?,?,?,?)
                    """,
                    (iid, inc_oem, inc_env, "", inc_sev, inc_summary, date.today().isoformat()),
                )
